...
*/
@WebServlet(name = "ValidateUserIdServlet", urlPatterns = {"/validateId"})
  ...
  */
  protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    try (PrintWriter out = response.getWriter();) {
      ...
      // récupère la valuer du paramètre "id" du formulaire
      String targetId = request.getParameter("id");

      //  renvoie un message XML qui sera soit  "<valid>true</valid>" soit
      //  <valid>false</valid>"  selon que id est ou nnom déjà utilisé.
      //  noter que le type MIME du contenu est "text/xml".
      try {
        String rep;
        if (targetId != null && LoginManager.validateUserId(this.ds, targetId)) {
        ...
        Logger.getLogger(ValidateUserIdServlet.class.getName()).log(Level.SEVERE, null, ex);
